...
export async function getTablaPosiciones(torneoId: string): Promise<TablaRow[]> {
  return extractData(api.get(`/stats/tabla/${torneoId}`));
}

export async function getGoleadores(torneoId: string): Promise<GoleadorRow[]> {
  return extractData(api.get(`/stats/goleadores/${torneoId}`));
}

export async function getTarjetas(torneoId: string): Promise<TarjetaRow[]> {
  return extractData(api.get(`/stats/tarjetas/${torneoId}`));
}

export async function getResumenTorneo(torneoId: string): Promise<ResumenTorneo> {
